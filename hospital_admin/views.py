...
def labworker_dashboard(request):
    if request.user.is_authenticated:
        if request.user.is_labworker:
            ...
            total_labworkers_count = Clinical_Laboratory_Technician.objects.annotate(count=Count('technician_id'))
            total_test_count = Test.objects.annotate(count=Count('test_id'))
            total_reports_count = Report.objects.annotate(count=Count('patient_id'))
            total_patient_count = Patient.objects.annotate(count=Count('patient_id'))

            medicine = Medicine.objects.all()
            
            context = {'doctor': doctor,'lab_workers':lab_workers,
                    'total_labworkers_count':total_labworkers_count,
...
def logout_view(request):
    ...
    messages.error(request, "User logged out.")
    return redirect('/hospital_admin/login/')
...
def got_offline(sender, request, user, **kwargs):
    if user is not None:
        try:
            ...
            print("Error updating login_status:", e)